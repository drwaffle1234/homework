...
public class Turtlet extends Object {
	...
	public static final Color RED = Color.red, BLUE = Color.blue,
			BLACK = Color.black, GRAY = Color.gray, YELLOW = Color.yellow,
			PINK = Color.pink, ORANGE = Color.orange, GREEN = Color.green,
			MAGENTA = Color.magenta, WHITE = Color.white;
	private static java.awt.Graphics thePage;
	// ////////////////////////////////
	protected double heading = 0; // heading initially east
	protected double xcor, ycor; // current position of Turtle
	Random gen = new Random();

	/**
	 * Set the drawing Color to the given value. Made an instance method only so
	 * that it cannot be called until thePage is assigned, although the drawing
	 * color is a property of thePage, not of one Turtle.
	 */
	public void RandomMoveTo() {
		int number = gen.nextInt(760);
		int number1 = gen.nextInt(600);
	...
	public void swingAround(double radius) {
		if (radius > 0.0) {
			...
			thePage.drawOval(round(xcor - radius), round(ycor - radius), rad,
					rad);
		}
	} // ======================
	...
	public void fillCircle(double radius) {
		if (radius > 0.0) {
			int rad = round(2 * radius);
			thePage.fillOval(round(xcor - radius), round(ycor - radius), rad,
					rad);
		}
	} // ======================
	...
	public Turtlet paint(double left, double forward) {
		...
		thePage.drawLine(round(xcor), round(ycor), round(x), round(y));
		xcor = x;
		ycor = y;
		return this;
	} // ======================
	...
	public void fillBox(double width, double height) {
		if (width > 0.0 && height > 0.0) {
			thePage.fillRect(round(xcor - width / 2), round(ycor - height / 2),
					round(width), round(height));
		}
	} // ======================

	/**
	 * Pause the animation for wait milliseconds. Made a class method so that an
	 * applet can pause an animation "between turtles".
	 */
	...
	/**
	 * Create a Turtlet on a given Component at a given starting position. All
	 * Turtlets must be created from within the Component's paint() method or
	 * from a method called by it. All Turtles live in the same world at any
	 * given time, so changing the page is analogous to spaceshipping the entire
	 * Turtle population to a new world.
	 */
